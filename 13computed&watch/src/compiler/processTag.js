...
export function processTag(ast, stack) {
...
function processDirectives(ast, attr) {
  if (attr.name === 'v-model' && ast.tag === 'input') {
...
export function processText(ast) {
  if (ast.type === 2) {
...
function processExpress(text) {
  let preIndex = 0, index, tokens = [], match
  while (match = moustacheRE.exec(text)) {
    index = match.index
    if (index > preIndex) {
      tokens.push(JSON.stringify(text.slice(preIndex, index)))
      tokens.push(`_s(${match[1]})`)
      preIndex = index + match[0].length
    } else {
      tokens.push(`_s(${match[1]})`)
      preIndex = index + match[0].length
    }
