import { ownHas } from '../shared/index'
import { processTag, processText } from './processTag'

// 匹配起始标签 比如 <div   <p
const startTagRe = /^<([0-9a-zA-Z-]+)(?=[\s\/"'>])/
...
// parser 函数 ，将文本字符串转换成 AST 
export function parser(temStr) {
  let textEnd
  while (temStr) {
  ...
  function parseStarTag() {
    let start = temStr.match(startTagRe)
    if (start) {
...
function handleEnd() {
  if (stack.length > 1) {
    ...
    if (ownHas(startTag, 'else') || ownHas(startTag, 'elseif')) return
